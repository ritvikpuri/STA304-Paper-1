---
title: "Toronto Apartment Evaluations Are Hard to Predict"
...
We have taken Apartment Evaluation Score Data from Open Data Toronto(REF) and we will using this to gain a better understading of whether there have been anay improvements in the apartment building conditons. The data almost 10,000 entries spread over 40 columns. These columns give us a variety of information about each evaluation conducted, ranging from year built, year evaluated, property type, score, building address, wardnames, confirmed storeys, confimred units,stairwells, security rooms, laundry rooms, number of entrances, exits and a lot more. (REF)

To analyze our dataset, I will be using R (REF), tidyverse (REF), dyplr (REF). There were multiple steps I took to clean and extract the neccesary data. The first thing I did was change the neccesssary column types from character to numeric, because initally columns such as SCORE and WARD were character type and it would not have been possible to graph them. Since we have a lot more entries of PRIVATE property type than PUBLIC and TCHC property types, I filtered out all entries that were not PRIVATE. Thus we will only be assesing the building evaluations for private apartment buildings.

First I decide to take a look at the average scores of all the buildings based on the categories defined by the RentSafeTO Program (Ref). This meant categorizng scores according to the ranges they mentioned (Figure 1). To cateogirze the scores I added another column to the database that gave the cateogry based on their scores. I also reported the max, min, mean and standard deviation of all the scores in our dataset. (Table 1). Along with these two plots, I also made a bargraph based on the avereage scores of each of the 25 wards the apartment buildings are classified into. (Figure 2)

The next four graphs are to see what helps buildings get a higher score than the rest. Since we have a lot of columns, I decided to focus on the most basic ones. Namely, Stairwells, Security, Laundry Rooms and Elevators. I made scatterplots using ggplot(REF) to see if any of these factors promote scores.
```{r}
...
#Figure 1
mut_data %>%
  ggplot(mapping = aes(x = score_category)) +
...
#Figure 3
data %>%
  ggplot(mapping = aes(x = SCORE, y = STAIRWELLS, color = WARDNAME)) +
  geom_point()

data %>%
  ggplot(mapping = aes(x = SCORE, y = ELEVATORS, color = WARDNAME)) +
  geom_point()

data %>%
  ggplot(mapping = aes(x = SCORE, y = SECURITY, color = WARDNAME)) +
  geom_point()

data %>%
  ggplot(mapping = aes(x = SCORE, y = LAUNDRY_ROOMS, color = WARDNAME)) +
  geom_point()
